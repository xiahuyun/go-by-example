...
if curl -fsS "http://${ETCD_ENDPOINT}/health" >/dev/null 2>&1; then
  ...
  "${ETCD_BIN}" \
    ...
    --initial-cluster "mvcc-demo=http://${ETCD_HOST}:${ETCD_PEER_PORT}" \
    --initial-cluster-state new \
    --initial-cluster-token mvcc-demo-token \
    >"${ETCD_LOG_FILE}" 2>&1 &
...
echo "[info] running MVCC demo..."
(
  cd "${SCRIPT_DIR}"
  ETCD_ENDPOINTS="${ETCD_ENDPOINT}" go run .
)

echo "[done] mvcc demo completed"
